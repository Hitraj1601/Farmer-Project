...
const CATEGORY_KEYWORDS = {
  Grains: ['wheat', 'rice', 'basmati', 'maize', 'corn', 'bajra', 'barley', 'oat', 'millet', 'grain'],
  Vegetables: ['tomato', 'potato', 'onion', 'brinjal', 'cauliflower', 'cabbage', 'spinach', 'capsicum', 'carrot', 'radish', 'ginger', 'garlic', 'broccoli', 'peas', 'beans', 'vegetable', 'greens'],
  Fruits: ['banana', 'mango', 'alphonso', 'apple', 'orange', 'grape', 'pomegranate', 'guava', 'watermelon', 'papaya', 'lemon', 'lime', 'cherry', 'berry', 'fruit'],
  Spices: ['turmeric', 'chilli', 'chili', 'coriander', 'cumin', 'cardamom', 'pepper', 'clove', 'cinnamon', 'mustard', 'nutmeg', 'spice'],
  Pulses: ['moong', 'chana', 'toor', 'dal', 'urad', 'masoor', 'lentil', 'chickpea', 'pulse'],
  Oilseeds: ['soybean', 'groundnut', 'peanut', 'mustard seed', 'sesame', 'sunflower', 'canola', 'oilseed'],
  Dairy: ['milk', 'ghee', 'paneer', 'butter', 'cheese', 'curd', 'yogurt', 'cream', 'dairy'],
  Flowers: ['flower', 'rose', 'marigold', 'jasmine', 'tulip', 'lily', 'sunflower', 'orchid'],
  'Cash Crops': ['cotton', 'sugarcane', 'tobacco', 'jute', 'rubber', 'fiber'],
  'Herbs & Medicinal': ['herb', 'aloe vera', 'tulsi', 'mint', 'neem', 'medicinal', 'ginseng', 'basil'],
  'Dry Fruits & Nuts': ['dry fruit', 'nut', 'cashew', 'almond', 'raisin', 'walnut', 'pistachio', 'dates', 'fig'],
  'Organic Manures': ['manure', 'compost', 'vermicompost', 'fertilizer', 'dung'],
};

const getSuggestedCategory = (cropName) => {
...
export default function CropForm() {
  ...
  const suggestedCategory = useMemo(() => {
    return getSuggestedCategory(form.cropName);
  }, [form.cropName]);
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 sm:p-8 space-y-6 animate-fade-in-up fill-mode-both delay-100">
        ...
        {suggestedCategory && suggestedCategory !== form.category && (
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-2xl p-4 border border-emerald-100 dark:border-emerald-800/30 animate-fade-in">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Suggested Category</p>
                  <p className="text-sm font-bold text-gray-800 dark:text-white">
                    We suggest <span className="text-emerald-600 dark:text-emerald-400">{suggestedCategory}</span> based on the crop name.
                  </p>
                </div>
              </div>
              <button
                ...
                className="px-3.5 py-1.5 text-xs font-bold bg-emerald-600 text-white hover:bg-emerald-700 rounded-xl transition-all shadow-[0_4px_12px_rgba(16,185,129,0.2)]"
              >
                Apply
              </button>
            </div>
